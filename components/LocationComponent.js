...
const LocationComponent = ({ headquarters, isLoading }) => (
  <section className="text-gray-700 body-font relative">
    {isLoading ? <Skeleton /> : <h1 className="text-center font-semibold text-2xl">Headquarters</h1>}
    <div className="container px-5 py-24 flex-col mx-auto flex sm:flex-no-wrap flex-wrap">
      <div className="bg-gray-300 rounded-lg overflow-hidden p-10 flex items-end justify-center relative">
        ...
        <div className="bg-white relative flex flex-wrap py-6 lg:w-1/2 md:w-2/3 w-full">
          <div className="lg:w-1/2 px-6">
            ...
            {isLoading ? <Skeleton /> : <p className="leading-relaxed">{headquarters.address}</p>}
          </div>
          <div className="lg:w-1/2 px-6 mt-4 lg:mt-0">
            <h2 className="title-font font-medium text-gray-900 tracking-widest text-sm">City</h2>
            {isLoading ? <Skeleton /> : <p className="text-blue-500 leading-relaxed">{headquarters.city}</p>}
            <h2 className="title-font font-medium text-gray-900 tracking-widest text-sm mt-4">State</h2>
            {isLoading ? <Skeleton /> : <p className="leading-relaxed">{headquarters.state}</p>}
          </div>
        </div>
      </div>
    </div>
